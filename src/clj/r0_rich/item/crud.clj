...
(defn change [params session]
  (if (:login session)
    (let [items (j/with-connection SQLDB 
          ...
          pic-name (:filename (:picture params))
          pic-path (if (empty? pic-name) nil
                       (str PRO_PIC_FOLDER "/" (:plucode params) "/" pic-name))]
      (if pic-path
        (do
          (io/delete-file (or (:picture (first items)) "") true)
          (io/make-parents pic-path)
          (io/copy (:tempfile (:picture params)) (io/file pic-path))))
      (let [update-fields (assoc (dissoc params :id) :picture (or pic-path (:picture (first items))))]
        (doseq [item items] 
          (j/update! SQLDB :Item update-fields (sql/where {:id (:id item)}))))
      (pages [:h2 "修改成功."]))
    (pages [:a {:href "/login"} "請登錄>>"])))

(defn- del-file-recur [f & [silently]]
  (let [f (io/file f)]
    (if (.isDirectory f)
      (doseq [child (.listFiles f)]
        (del-file-recur child silently)))
  ...
  (if (:login session)
    (do
      (j/with-connection SQLDB
        (let [plucode (:plucode (first (j/with-query-results rs [(str "select plucode from Item where id =" id ";")] (doall rs))))]
